...
function originAllowed(request, env) {
  ...
  // Non-browser clients may omit Origin. CORS still protects browser callers.
  if (!origin) {
    return true;
  }
...
async function checkSingleDomain(domain, apiKey) {
  ...
  if (!ahrefsResponse.ok) {
    ...
    } else if (ahrefsResponse.status >= 500) {
      errorMsg = "Ahrefs API is temporarily unavailable.";
    }
...
async function handleCheckDr(request, env) {
  if (!env.ALLOWED_ORIGIN) {
    return jsonResponse(
      { success: false, error: "ALLOWED_ORIGIN is not configured on the server." },
      500,
      request,
      env
    );
  ...
  if (body.domains.length > MAX_DOMAINS) {
    return jsonResponse(
      { success: false, error: `Maximum ${MAX_DOMAINS} domains are allowed.` },
      400,
      request,
      env
    );
  ...
  if (domains.length === 0) {
    return jsonResponse(
      { success: false, error: "No valid domains were provided." },
      400,
      request,
      env
    );
