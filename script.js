...
function renderResults() {
  ...
  pageResults.forEach((result, index) => {
    fragment.appendChild(createResultRow(result, startIndex + index));
  });
...
function updateSummary() {
  const successful = allResults.reduce(
    (count, result) => count + (result.status === "success" ? 1 : 0),
    0
  );
  const failed = allResults.length - successful;
  const ratingTotal = allResults.reduce(
    (sum, result) =>
      sum + (typeof result.domain_rating === "number" ? result.domain_rating : 0),
    0
  );
  const ratingCount = allResults.reduce(
    (count, result) => count + (typeof result.domain_rating === "number" ? 1 : 0),
    0
  );
  ...
  averageDr.textContent = ratingCount ? (ratingTotal / ratingCount).toFixed(1) : "—";
}
...
function applyFilterAndSort() {
  ...
  filteredResults = allResults
    .filter((result) => String(result.domain || "").toLowerCase().includes(query))
    .sort((a, b) => {
...
async function fetchWithTimeout(url, options = {}, timeoutMs = FETCH_TIMEOUT_MS) {
  ...
  try {
    ...
    if (error.name === "AbortError") {
      throw new Error("The request timed out. Please try again with fewer domains.");
    }
    throw error;
  } finally {
...
async function checkDomains() {
  ...
  if (!API_URL || API_URL.includes("YOUR-WORKER")) {
    showServerError("Please configure your Cloudflare Worker URL in script.js.");
    return;
  }
  ...
  try {
    ...
    console.error(error);
    showServerError(error.message || "Unable to check the domains. Please try again.");
  } finally {
    setLoading(false);
  }
}
...
function downloadCsv() {
  ...
  for (const result of allResults) {
    rows.push([result.domain, result.domain_rating ?? "", displayStatus(result.status)]);
  }
